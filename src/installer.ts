...
function generateDiscordPackageJson(): string {
...
async function getProcessesLockingDiscord(): Promise<ProcessInfo[]> {
  if (process.platform !== 'win32') {
  ...
  try {
    ...
    console.error('Error detecting Discord processes:', (e as Error).message);
    return [];
  }
}
...
async function killProcess(pid: number): Promise<boolean> {
  try {
    ...
    console.error(`Failed to kill process ${pid}: ${(e as Error).message}`);
    return false;
  }
}

// Kill Discord processes
async function killDiscord(): Promise<boolean> {
  ...
  if (processes.length === 0) {
    ...
    if (shouldClose) {
      ...
      try {
        ...
        console.error('Failed to close Discord:', (e as Error).message);
        return false;
      }
    } else {
...
function findDiscordPath(): string | undefined {
  let discordPath: string | undefined;
  ...
  if (process.platform === 'win32') {
    ...
    try {
      const discordBin = execSync('which discord').toString().trim();
      if (discordBin) {
...
function findAppDir(discordPath: string): string | undefined {
...
function findResourcesDir(appDir: string): string {
  return path.join(appDir, 'resources');
}

// Wait for file to be unlocked
async function waitForFileUnlock(filePath: string, maxAttempts: number = 10): Promise<boolean> {
...
async function inject(): Promise<void> {
  ...
  try {
    ...
    console.error('Error during injection:', (error as Error).message);
...
async function uninject(): Promise<void> {
  const discordPath = findDiscordPath();
  if (!discordPath) {
    console.error('Could not find Discord installation');
    return;
  ...
  try {
    ...
    console.error('Error during uninjection:', (error as Error).message);
    console.error('\nTroubleshooting:');
