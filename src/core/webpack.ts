...
function debugLog(area: string, message: string, ...data: any[]): void {
  console.log(`[JC:Debug][${area}] ${message}`, ...data);
}

// Interceptor class for webpack
export class WebpackInterceptor extends EventEmitter {
  private webpackRequire: any = null;
  private moduleCache: any = null;
  private ready = false;
  private moduleWaiters = new Map<(mod: any) => boolean, (mod: any, id: string) => void>();
  ...
  find(filter: (mod: any) => boolean, options: { silent?: boolean } = {}): any {
    if (!this.isReady()) {
      if (!options.silent) {
  ...
  findAll(filter: (mod: any) => boolean): any[] {
    if (!this.isReady()) {
  ...
  private processModule(exports: any, moduleId: string): void {
    if (!exports) return;
  ...
  private setupInterception(): void {
    ...
    const createFactoryProxy = (moduleId: string, factory: Function): Function => {
      // Skip if already proxied
      if ((factory as any)[SYM_IS_PROXIED_FACTORY]) {
        return factory;
      }
