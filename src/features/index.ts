...
export interface Feature {
  ...
  class FeatureRegistry {
    ...
    private settings: Map<string, Map<string, any>> = new Map();
    ...
    // Enable a feature
    async enableFeature(name: string): Promise<boolean> {
    ...
    // Disable a feature
    async disableFeature(name: string): Promise<boolean> {
      const feature = this.features.get(name);
      if (!feature) {
        console.warn(`[JC:Features] Feature "${name}" not found`);
        return false;
    ...
    // Get a setting value
    getSetting(featureName: string, settingId: string): any {
      const featureSettings = this.settings.get(featureName);
      if (!featureSettings) {
        const feature = this.features.get(featureName);
        if (!feature?.settings?.options) return undefined;
    ...
    updateSetting(featureName: string, settingId: string, value: any): void {
      const feature = this.features.get(featureName);
      if (!feature) {
    ...
    private saveSettings(): void {
      try {
        ...
        const settingsObj: Record<string, Record<string, any>> = {};
